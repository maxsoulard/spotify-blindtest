...
export const Mutation = {
  ...
  startNewGame: async (root, args, context) => {
    ...
    const response = await SpotifyApi.get(`/me/albums?limit=1`, { 'Authorization': context.spotifyUserAuthHeader }, context.logger);
    ...
    const firstAlbums = await SpotifyApi.get(`/me/albums?limit=20&offset=${firstOffset}`, { 'Authorization': context.spotifyUserAuthHeader }, context.logger);
    const secondAlbums = await SpotifyApi.get(`/me/albums?limit=20&offset=${secondOffset}`, { 'Authorization': context.spotifyUserAuthHeader }, context.logger);
